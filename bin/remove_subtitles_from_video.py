...
def detect_subtitle_region(frame, x=0.00065359477124183, y=0.7529411764705882, 
                          width=0.9952069716775599, height=0.05294117647058831,
                          detect_white_text=True, white_threshold=200):
    """
    ...
    if detect_white_text:
        # 提取字幕区域
        subtitle_region = frame[y_pixel:y_pixel+height_pixel, x_pixel:x_pixel+width_pixel]
        ...
        if subtitle_region.size > 0:
            ...
            mask[y_pixel:y_pixel+height_pixel, x_pixel:x_pixel+width_pixel] = combined_mask
    else:
        # 如果不检测白色，直接使用整个矩形区域
...
def process_frames_batch(model, frames, masks, device, pad_out_to_modulo=8, batch_size=4):
    """
    批量处理多帧图像（CPU优化版本）
    ...
    for batch_idx in tqdm(range(0, len(frames), batch_size), desc="处理批次", total=total_batches):
        ...
        # 堆叠成批次tensor
        img_tensor = torch.stack([torch.from_numpy(img).float() for img in batch_images])  # (B, 3, H, W)
        mask_tensor = torch.stack([torch.from_numpy(mask).float() for mask in batch_mask_tensors])  # (B, 1, H, W)
...
def extract_audio(video_path, audio_path):
    ...
    try:
        ...
        if 'no audio streams' in error_output.lower() or 'stream map' in error_output.lower():
            LOGGER.info("视频中没有音频流")
            return False
...
def create_video_from_frames(frame_paths, output_path, fps, width, height):
    ...
    LOGGER.info(f"创建视频: {len(frame_paths)}帧")
...
def main():
    ...
    for idx, frame_path in enumerate(tqdm(frame_paths, desc="准备帧")):
        frame = cv2.imread(frame_path)
        mask = detect_subtitle_region(frame, detect_white_text=True, white_threshold=200)
    ...
    LOGGER.info(f"需要处理的帧: {len(frames_to_process)}, 跳过的帧: {len(frames_to_skip)}")
    
    # 批量处理需要处理的帧
    if frames_to_process:
        LOGGER.info("批量处理帧...")
        processed_results = process_frames_batch(
            model, frames_to_process, masks_to_process, device, 
            pad_out_to_modulo=8, batch_size=batch_size
        )
        
        # 保存处理后的帧
        for (idx, frame_path), processed_frame in zip(frame_indices_to_process, processed_results):
